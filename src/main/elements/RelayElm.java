...
public class RelayElm extends CircuitElm {
	...
	void setPoints() {
		...
		for (i = 0; i != poleCount; i++) {
			...
			interpPoint(lead1, lead2, swpoles[i][1], 1,
					-openhs * 3 * i - openhs);
			interpPoint(lead1, lead2, swpoles[i][2], 1,
					-openhs * 3 * i + openhs);
			interpPoint(point1, point2, swposts[i][0], 0, -openhs * 3 * i);
			interpPoint(point1, point2, swposts[i][1], 1,
					-openhs * 3 * i - openhs);
			interpPoint(point1, point2, swposts[i][2], 1,
					-openhs * 3 * i + openhs);
		}
	...
	void calculateCurrent() {
		...
		for (p = 0; p != poleCount; p++) {
			if (i_position == 2)
				...
				switchCurrent[p] = (volts[nSwitch0 + p * 3]
